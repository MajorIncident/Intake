...
let hasDiffTarget = false;

try {
  runGit(`git rev-parse --verify ${diffTarget}`, { quiet: true });
  hasDiffTarget = true;
} catch (error) {
  console.warn(`[verify:persistence] ${diffTarget} not found locally; fetching ${baseRef}...`);
  try {
    runGit(`git fetch --no-tags origin ${baseRef}:${baseRef}`, { quiet: true });
    runGit(`git rev-parse --verify ${baseRef}`, { quiet: true });
    diffTarget = baseRef;
    hasDiffTarget = true;
  } catch (fetchError) {
    console.warn('[verify:persistence] Unable to fetch the base branch. Falling back to working tree changes if needed.');
    diffTarget = baseRef;
  }
}
...
function getChangedFiles() {
  ...
  if (hasDiffTarget) {
    try {
      ...
      // Proceed to working tree fallback below.
    }
  }
  if (!hasDiffTarget || output === '') {
    try {
...
function getInterfaceAdditions() {
  ...
  if (hasDiffTarget) {
    ...
    try {
      ...
      // Fall through to working tree diff below.
    }
  }
  if (diff === '') {
    try {
