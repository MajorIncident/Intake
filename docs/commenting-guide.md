...
| `[feature:collaboration-presence]` | `index.html`, below the primary header | Shows the editable shared team name and all recently active participants, and provides the accessible start/join/edit identity dialog. | `src/collaboration.js`, `api/workspaces/presence.js` | `collaborationWorkspace`, `collaborationParticipants`, `collaborationDialog`; local identity preference uses `kt-collaboration-profile-v1`, team metadata stays server-side, and both are excluded from intake snapshots. |
