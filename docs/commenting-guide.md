...
| `[feature:major-incident-role-legend]` | `index.html`, below the header | Provides the Major Incident-only phase legend mount; contextual heading and KT-row badges are rendered alongside it. | `src/majorIncidentRoles.js`, `src/intakeModes.js` | `majorIncidentRoleLegend`, `majorIncidentRoleLegendItems`; presentational only and not persisted. |
