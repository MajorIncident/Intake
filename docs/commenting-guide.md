...
| `[feature:collaboration-presence]` | `index.html`, below the primary header | Shows the shared team name and recently active participants, and provides the accessible start/join/edit identity dialog. | `src/collaboration.js`, `api/workspaces/presence.js` | `collaborationWorkspace`, `collaborationParticipants`, `collaborationDialog`; local identity preference uses `kt-collaboration-profile-v1` and is excluded from intake snapshots. |
