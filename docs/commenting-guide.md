...
| `[header]` | `index.html` | Encapsulates the hero title plus toolbar controls for collaboration, comms, steps, file transfer, templates, and summary triggers; Collaboration remains available in every intake mode while comms and steps are progressively disclosed. | `src/commsDrawer.js`, `src/steps.js`, `src/fileTransfer.js`, `src/templatesDrawer.js`, `src/summary.js`, `src/intakeModeController.js` | `docTitle`, `intakeModeSelect`, `commsBtn`, `stepsBtn`, `stepsDrawer`, `stepsCompletedLabel`, `startFreshBtn`, `saveToFileBtn`, `loadFromFileBtn`, `genSummaryBtn`; persisted via `kt-intake-full-v2`. |
| `[feature:collaboration]` | `index.html`, Collaboration menu | Provides secret-link session actions, synchronization status, and conflict recovery choices in every intake mode. | `src/collaboration.js`, `api/workspaces/*` | `collaborationStatus`, session action buttons; recovery uses `kt-collaboration-recovery-v1` while intake snapshots remain under `kt-intake-full-v2`. |
| `[feature:collaboration-presence]` | `index.html`, in the workspace dock immediately before Notes workspace | Shows the editable shared team name and all recently active participants, and provides the accessible start/join/edit identity dialog. | `src/collaboration.js`, `api/workspaces/presence.js` | `collaborationWorkspace`, `collaborationParticipants`, `collaborationDialog`; local identity preference uses `kt-collaboration-profile-v1`, team metadata stays server-side, and both are excluded from intake snapshots. |
...
| `[feature:intake-mode-selector]` | `index.html` header title | Wraps the accessible intake mode selector used to choose between General, IT, Pharma, and Major Incident Management lenses before capture. | `src/intakeModes.js`, `src/intakeModeController.js` | `intakeModeSelect`; options use stable mode values and persist under `meta.intakeMode` in `kt-intake-full-v2`. |
| `[feature:major-incident-role-legend]` | `index.html`, below the header | Provides the Major Incident-only four-phase legend mount; phase/primary-role badges decorate mapped workflow headings (including Decision and Risk gates) and every KT row, then hide without unmounting data when the mode changes. Icons and colour swatches are decorative because visible and accessible role text carries their meaning. | `src/majorIncidentRoles.js`, `src/intakeModes.js` | `majorIncidentRoleLegend`, `majorIncidentRoleLegendItems`; role presentation is not persisted, while the decorated controls retain their normal field contracts. |
| `[feature:notes-workspace]` | `index.html`, beside the main wrap | Always-mounted, non-modal persistent note dock available in every intake mode. | `src/notesWorkspace.js`, `src/appState.js` | `notesWorkspace`, `notesWorkspaceInput`, `notesWorkspaceList`, `notesWorkspaceToggle`; persists as `notesWorkspace.notes` and `notesWorkspace.open` under `kt-intake-full-v2`; launcher shortcut is Alt+N. |
| `[section:preface]` | `index.html` main wrap | Groups the problem summary, object/deviation notes, detection timeline, incident proof, and baseline evidence fields; detection, evidence-collected, and proof controls are mode-controlled Major Incident-only regions. | `src/preface.js`, `src/appState.js`, `src/intakeModeController.js` | `oneLine`, `objectPrefill`, `proof`, `detectMonitoring`, `detectUserReport`, `detectAutomation`, `detectOther`, `evScreenshot`, `evLogs`, `evMetrics`, `evRepro`, `evOther`, `healthy`, `now`; all serialized into `kt-intake-full-v2`. |
...
| `[feature:major-incident-analysis]` | `index.html`, after Possible Causes | Progressively discloses Major Incident Decision Analysis and Potential Problem Analysis cards. | `src/majorIncidentAnalysis.js`, `src/actionsStore.js`, `src/appState.js` | `decisionAnalysis` plus action-compatible `potentialProblemAnalysis.owner`, `.risk`, `.changeControl`, and `.verification`; persisted in `kt-intake-full-v2`. |
