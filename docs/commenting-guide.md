...
| `[feature:intake-mode-selector]` | `index.html` header title | Wraps the accessible intake mode selector used to choose between General, IT, Pharma, and Major Incident Management lenses before capture. | `src/intakeModes.js` | `intakeModeSelect`; options use stable mode values and currently do not add a storage key. |
