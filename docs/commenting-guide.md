...
| `[header]` | `index.html` | Encapsulates the hero title plus toolbar controls for comms, steps, file transfer, templates, and summary triggers; the Collaboration menu is marked as a mode-controlled Major Incident-only region. | `src/commsDrawer.js`, `src/steps.js`, `src/fileTransfer.js`, `src/templatesDrawer.js`, `src/summary.js`, `src/intakeModeController.js` | `docTitle`, `intakeModeSelect`, `commsBtn`, `stepsBtn`, `stepsDrawer`, `stepsCompletedLabel`, `startFreshBtn`, `saveToFileBtn`, `loadFromFileBtn`, `genSummaryBtn`; persisted via `kt-intake-full-v2`. |
| `[feature:collaboration]` | `index.html`, Collaboration menu | Provides secret-link session actions, synchronization status, and conflict recovery choices. | `src/collaboration.js`, `api/workspaces/*` | `collaborationStatus`, session action buttons; recovery uses `kt-collaboration-recovery-v1` while intake snapshots remain under `kt-intake-full-v2`. |
