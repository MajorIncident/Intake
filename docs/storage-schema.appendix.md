...
- **Notes:** The component writes to the dedicated `kt-actions-by-analysis-v1` key and mirrors data into the main snapshot through `exportActionsState()` so Save/Load flows stay portable. `actions.items` should always be generated by `listActions()` / `exportActionsState()`.
