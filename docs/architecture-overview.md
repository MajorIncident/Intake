...
| `src/collaboration.js` | Owns `[feature:collaboration]` and `[feature:collaboration-presence]`, revision-safe snapshot synchronization, team identity dialogs, participant presence heartbeats, and conflict recovery. | Receives `collectAppState`, `applyAppState`, local-save, and toast callbacks from `main.js`; presence uses a separate API and `kt-collaboration-profile-v1` so identity changes never alter intake revisions. |
